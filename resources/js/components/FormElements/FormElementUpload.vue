<template>
  <div class="v-full">
    <div class="w-full">
      <label
        class="max-w-xs w-full py-6 px-3 flex flex-wrap flex-row form-input-bordered border border-dashed cursor-pointer"
      >
...
export default {
  ...
  computed: {
    extensions() {
      return _.join(
        _.map(this.currentField.extensions, (extension) => {
          return `.${extension}`;
        }),
        ','
      );
